...
function maskPreventEvent(ee) {
  ...
  if (utils.browserIsMobile()) {
    dom.removeEventListener(ee, "touchmove", maskPreventHandler);
    dom.addEventListener(ee, "touchmove", maskPreventHandler);
    dom.removeEventListener(ee, "touchup", maskPreventHandler);
    dom.addEventListener(ee, "touchup", maskPreventHandler);
    dom.removeEventListener(ee, "touchdown", maskPreventHandler);
    dom.addEventListener(ee, "touchdown", maskPreventHandler);
  } else {
    dom.removeEventListener(ee, "mousewheel", maskPreventHandler);
    dom.addEventListener(ee, "mousewheel", maskPreventHandler);
    dom.removeEventListener(ee, "mouseover", maskPreventHandler);
    dom.addEventListener(ee, "mouseover", maskPreventHandler);
  }

  dom.removeEventListener(ee, "click", maskPreventHandler);
  dom.addEventListener(ee, "click", maskPreventHandler);
}
/**
 * @desc: 遮罩层当前的zindex.
 */
...
function showWidget(el, showMask, preventEvent, hideBodyScroll, cb) {
  ...
  if (hideBodyScroll) {
    ...
    if (showMask || preventEvent) {
      // 桌面端判断垂直滚动条.
      if (!utils.browserIsMobile()) {
        scrollWidth = window.innerWidth - dom.getViewPort().width; // if (scrollWidth > 0) {
        //   willFix = true;
        // }

        if (dom.getDocumentPort().height > dom.getViewPort().height) {
          willFix = true;
        }
      } // 移动端
      else {
  ...
  bpLibs.dom.probeDom(200, function () {
    ...
    return 0 == window.innerWidth - dom.getViewPort().width;
  }.bind(this), function () {
    var _this2 = this;

    _newArrowCheck(this, _this);

    utils.sleep(0).then(function () {
      ...
      return utils.sleep(10).then(function () {
        ...
        if (clientHeight > dom.getViewPort().height) {
          clientHeight = (clientHeight - dom.getViewPort().height) / 2 + 20;
          $(content).css('top', clientHeight + 'px');
        }

        return utils.sleep(duration - 10);
      }.bind(this));
    }.bind(this)).then(function () {
      _newArrowCheck(this, _this2);
...
loading:any,
  ...
  window[GlobalDialogComponents] = utils.mergeMap(window[GlobalDialogComponents], cfg);
}
...
confirm?: ()=>void,
  ...
  cfg.title = cfg.title || '';
  cfg = utils.mergeMap(alert.data(), cfg);
  var c = utils.mergeMap(alert, {
    data: function data() {
      _newArrowCheck(this, _this2);
...
cancel?: ()=>void,
  ...
  cfg.title = cfg.title || '';
  cfg = utils.mergeMap(confirm.data(), cfg);
  var c = utils.mergeMap(confirm, {
    data: function data() {
  ...
  return new Promise(function (resolve, reject) {
    ...
    vm.$children[0].$children[0].show().then(function (res) {
      _newArrowCheck(this, _this7);
    }.bind(this)); // return { vm: vm.$children[0].$children[0], id };
  }.bind(this));
}
/**
...
delay: 延时显示, 默认为0.
  ...
  if (window[GlobalLoadingTimeout]) {
    if (window[GlobalLoadingTimeout].now > now + cfg.delay) {
      ...
      window[GlobalLoadingTimeout].cfg = utils.mergeMap(cfg);
      return;
    }
  }
