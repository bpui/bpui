  ...
  <div>
    ...
    <bpSelect ref="selectc" @change="onChange" @blur="onChange" multiple clearable>
      <bp-select-option value="1">
        <div style="align-items: center;display: inline-flex; width:auto">
          <bp-icon name="loading" width="20px" />1 dsfds</div>
      </bp-select-option>
    ...
    <bpSelect>
      <bp-select-option value="1">
        <div style="align-items: center;display: flex; width:auto">
          <bp-icon name="loading" width="20px" style="margin-right:5px;" />1 dsfds</div>
      </bp-select-option>
    ...
    <h4>multiple</h4>
    <bpSelect ref="select1" v-model="singleValue" multiple clearable :datasource="singleDatasource"
      @change="onChange"></bpSelect>
    ...
    <button
      @click="$bpWidget.showAlert(JSON.stringify($refs.select1.getSelect(0)))">getSelect</button>
    ...
    <button
      @click="$bpWidget.showAlert(JSON.stringify($refs.selectSingle.getSelect(0)))">getSelect</button>

    <h4>cascader</h4>
    <bpSelect ref="select2" v-model="cascaderValue" multiple :datasource="cascaderDatasource">
    </bpSelect>
    ...
    <button
      @click="$bpWidget.showAlert(JSON.stringify($refs.select2.getSelect(0)))">getSelect</button>

    <h4>empty</h4>
    <bpSelect ref="select3" :datasource="[]"></bpSelect>

    <h4>PickerDatasource</h4>
    <bpSelect ref="select4" v-model="cascaderValue2" sepText="-" :datasource="PickerDatasource">
    </bpSelect>
...
<script lang="ts">
  ...
  export default class App extends Vue {
    ...
    singleFilerableValue = '';
    singleFilerableDatasource = new Array(10).fill(1).map((o, i) => ({
      label: `label${i + 1}`,
      value: `value${i}`,
      id: i
    }));
    ...
    singleRemoteDatasource = new Array(10).fill(1).map((o, i) => ({
      label: `label${i + 1}`,
      value: `value${i}`,
      id: i
    }));
    ...
    multipleDatasource = new Array(10).fill(1).map((o, i) => ({
      label: `label${i + 1}`,
      value: `value${i}`,
      id: i
    }));
    ...
    multipleFilterDatasource = new Array(10).fill(1).map((o, i) => ({
      label: `label${i + 1}`,
      value: `value${i}`,
      id: i
    }));
    ...
    multipleRemoteDatasource = new Array(10).fill(1).map((o, i) => ({
      label: `label${i + 1}`,
      value: `value${i}`,
      id: i
    }));
